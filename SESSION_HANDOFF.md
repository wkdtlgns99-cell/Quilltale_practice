...
- [ ] **30. 수면 결핍 & 생체 각성 시계 엔진 (`SleepDeprivationEngine`)**:
  - **구현 대상**: [엔진: 신규] `src/world/sleep_engine.py`
  - **기능**: 24시간 이상 미수면 시 집중력 저하(명중/회피 -3), 36시간 미수면 시 섬망/헛소리, 48시간 이상 미수면 시 마이크로슬립(이동/전투 중 강제 수면 기습 허용) 및 혼절.
