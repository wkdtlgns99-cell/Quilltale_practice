...
- [ ] **31. 전장 시체 부패 & 청소 야수/사령 유인 생태계 (`CorpseEcologyEngine`)**:
  - **구현 대상**: [엔진: 신규] `src/world/corpse_ecology_engine.py`
  - **기능**: 전투 승리 후 방치된 시체 3~6시간 경과 시 부패 악취 확산, 주변 스캐벤저(늑대/까마귀/구울) 유인 스폰 및 전리품 훼손.
- [ ] **32. 포션 약물 남용 간 독성 & 내성 축적 엔진 (`ToxicologyToleranceEngine`)**:
  - **구현 대상**: [엔진: 신규] `src/world/toxicology_engine.py`
  - **기능**: 회복 물약 연속 음용 시 체내 독성(Toxicity) 누적, 100 도달 시 구토 및 장기 손상/탈진, 빈번한 복용 시 내성 형성(효과 반감).
- [ ] **33. 급격한 명암 변화 안구 암적응/명적응 물리 엔진 (`PupilAdaptationEngine`)**:
  - **구현 대상**: [엔진: 신규] `src/world/pupil_adaptation_engine.py`
  - **기능**: 대낮 야외에서 어두운 동굴 진입 시 2턴간 동공 미확장 일시적 실명(명중 DC+6), 어둠 속 횃불/섬광 노출 시 순간 눈부심(Flash Blindness) 1턴 무력화.
- [ ] **34. 🔥 [신규 백로그] 알코올 취기 & 숙취 중독 물리 엔진 (`AlcoholIntoxicationEngine`)**:
  - **구현 대상**: [엔진: 신규] `src/world/alcohol_engine.py`
  - **기능**: 주점 맥주/와인/독주 섭취 시 혈중 알코올 농도(BAC), 1단계(기분 고양, 공포/스트레스 완화, 명중 -1), 2단계(혀 꼬임, 보행 비틀거림, 민첩 -3), 3단계(필름 끊김, 인사불성, 노상 실신 및 소지품 도난), 기상 시 숙취 두통/탈수 디버프.
- [ ] **35. 🔥 [신규 백로그] 야영 모닥불 & 침낭/숙영지 방어 엔진 (`CampsiteRestEngine`)**:
  - **구현 대상**: [엔진: 신규] `src/world/campsite_engine.py`
  - **기능**: 모닥불 점화 시 건조/보온 유지 vs 야수/도적 유인 어그로 배율, 방어 목책/경보 덫 설치, 파티원 불침번 3교대 경계 근무 및 피로도 불완전 회복.
- [ ] **36. 🔥 [신규 백로그] 독초 감별 & 약초 채집 야생 식물학 엔진 (`HerbalismBotanyEngine`)**:
...
- [ ] **37. 🔥 [신규 백로그] 마나 회로 손상 수술 & 에테르 정화 치료 엔진 (`ManaVeinRestorationEngine`)**:
  - **구현 대상**: [엔진: 신규] `src/world/vein_restoration_engine.py`
  - **기능**: 마나 폭주로 파열된 회로(Vein Scarring)를 정밀 비전 수술/은침/특수 탕약으로 복구, 에테르 침전물 투석, 고위 비전 의사 NPC 퀘스트라인.
- [ ] **38. 🔥 [신규 백로그] 기상 제어 마법 & 시공간 환경 재해 시뮬레이션 엔진 (`WeatherMagicSimulationEngine`)**:
  - **구현 대상**: [엔진: 신규] `src/world/weather_magic_engine.py`
  - **기능**: 고드름 비/폭풍우/태양빛 소환 등 환경 제어 마법, 전투 중 턴당 피격 판정 vs 탐험 이동 중 소요 시간(분/시간) 비례 누적 틱 피해 및 저체온/피로 정산, 마법 기상 지속시간 감쇄.
...
### 2. 테스트 및 평가 검증 상태
- **프로젝트 전체 413개 단위 테스트 100% 무결점 통과 (회귀 결함 0건)**:
  - `tests/test_epidemic_and_gear.py`: 6개 신규 단위 테스트 통과 (질병 스펙/traits, 체질 세이빙 스로우 및 대성공/대실패, 잠복기 및 1~3단계 진행, 치료제/소작/소각, 가방 규격/과적 찢어짐 물리, 스킬 외형 연출).
  - `pytest tests/`: **413 passed in 4.82s**.
- **DoD Gate Eval Runner 검증**:
  - `python eval_runner.py --no-judge` (20턴): **`Invalid transition rate: 0.0%`** 달성.

### 3. 다음 세션 작업 착수 안내 (Next Step)
- **현재 완료 상태**:
  - 스태미너 시스템 + 던전 탐험 계층 시스템 + 3대 맵 컨셉 함정 엔진 + 파티원 멘탈 붕괴 & 스트레스 엔진 + 던전 구조적 붕괴 & 산소 고갈 질식 엔진 + 날씨·체온 저체온증/열사병 생존 물리 엔진 + 전염병 생체 엔진 + 가방 규격/찢어짐 + 스킬 외형 연출 100% 완공.
- **다음 작업 후보**:
  - 후보 1: **마나 과부하 폭주 & 에테르 오염 변이 엔진 (`ManaBurnEngine` / 백로그 8번)** (최우선 권장)
  - 후보 2: **식량 부패·수질 오염 & 보존식 염장 가공 엔진 (`RationSpoilageEngine` / 백로그 16번)**
  - 후보 3: **수면 결핍 & 생체 각성 시계 엔진 (`SleepDeprivationEngine` / 백로그 30번)**
  - 후보 4: **성벽 공성전 & 대규모 전열 전술 엔진 (`SiegeWarfareEngine` / 백로그 9번)**
